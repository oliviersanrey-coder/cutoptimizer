...
def pack_rip_first(panel: PanelSpec, pieces: List[PieceInstance]) -> PanelLayout:
    ...
    for p in pieces_sorted:
        cands = []
        for L, W, rot in oriented_candidates(p):
            if L <= UL and W <= UW:
                cands.append((L, W, rot))
        if not cands:
            continue

        best_choice = None
        # score: (creates_new_strip, strip_height, wasted_x_after, y_pos)
...
def pack_crosscut_first(panel: PanelSpec, pieces: List[PieceInstance]) -> PanelLayout:
    ...
    for p in pieces_sorted:
        ...
        best_choice = None
        # score: (creates_new_band, band_length, wasted_y_after, x_pos)
        for L, W, rot in cands:
            for b in bands:
                if b["w"] != L:
                    continue
